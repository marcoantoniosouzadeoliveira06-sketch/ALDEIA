...
(function () {
    ...
    function initDomeGallery() {
        ...
        function handleResize() {
            ...
            let radius = basis * 0.5; // fit
            const heightGuard = h * 1.35;
            radius = Math.min(radius, heightGuard);
            radius = Math.max(600, radius); // minRadius = 600
            ...
            root.style.setProperty('--overlay-blur-color', '#080808');
            root.style.setProperty('--tile-radius', '30px');
            root.style.setProperty('--enlarge-radius', '30px');
            root.style.setProperty('--image-filter', 'none'); // Full color, no grayscale!
            
            applyTransform(rotation.x, rotation.y);
            
            // Re-align enlarge wrapper if zoom is active
            ...
            if (enlargedOverlay) {
                ...
                tempDiv.style.cssText = `position: absolute; width: 250px; height: 350px; visibility: hidden;`;
        ...
        function startInertia(vx, vy) {
            const MAX_V = 1.4;
            let vX = Math.min(Math.max(vx, -MAX_V), MAX_V) * 80;
            let vY = Math.min(Math.max(vy, -MAX_V), MAX_V) * 80;
            let frames = 0;
            const frictionMul = 0.94 + 0.055 * 0.6; // using dampening
            const stopThreshold = 0.015 - 0.01 * 0.6;
            const maxFrames = 90 + 270 * 0.6;
            ...
            function step() {
                ...
                const nextX = Math.min(Math.max(rotation.x - vY / 200, -maxVerticalRotationDeg), maxVerticalRotationDeg);
                const nextY = (((rotation.y + vX / 200 + 180) % 360) + 360) % 360 - 180;
        ...
        // Pointer Drag Events
        let velocityTracker = { x: 0, y: 0, lastTime: 0, lastX: 0, lastY: 0 };
        ...
        window.addEventListener('pointermove', e => {
            ...
            const nextX = Math.min(Math.max(startRot.x - dyTotal / dragSensitivity, -maxVerticalRotationDeg), maxVerticalRotationDeg);
            const nextY = (((startRot.y + dxTotal / dragSensitivity + 180) % 360) + 360) % 360 - 180;
            ...
            const dt = now - velocityTracker.lastTime;
            if (dt > 10) {
                velocityTracker.x = (e.clientX - velocityTracker.lastX) / dt;
                velocityTracker.y = (e.clientY - velocityTracker.lastY) / dt;
                velocityTracker.lastTime = now;
                velocityTracker.lastX = e.clientX;
                velocityTracker.lastY = e.clientY;
            }
        });
        ...
        window.addEventListener('pointerup', e => {
            ...
            if (moved) {
                ...
                // If recently moved, apply inertia
                const dt = now - velocityTracker.lastTime;
                if (dt < 100) {
                    startInertia(velocityTracker.x, velocityTracker.y);
                }
            }
            moved = false;
            startPos = null;
